...
def testmodels(X_tr, y_tr, X_te, y_te, archs, actfn='relu', last_act='softmax', reg_coeffs=[0.0], 
	...
	[n_tr,d] = X_tr.shape
	[n_te,d] = X_te.shape
	print 'testmodel :: ' , X_tr.shape, X_te.shape
	best_acc = 0
	best_config = []
				...
				for sgd_mom in sgd_moms:
					...
					if EStop:
						...
						model.fit(X_tr, y_tr, nb_epoch=num_epoch, batch_size=batch_size, 
							verbose=verbose)
					# Evaluate Models
					y_predict = model.predict(X_te, batch_size=batch_size, verbose=verbose)
					#score = model.evaluate(X_te, y_te, batch_size=batch_size, verbose=verbose)
					out_filename=str(arch).replace(',','_') +'.csv'
					print out_filename,y_predict
					#y_predict.to_csv(out_filename, sep='\t')
					np.savetxt(out_filename,y_predict)
					'''
					if score[1] > best_acc:
						best_acc = score[1]
