...
arch_list_e=[[din, din, dout],
             [din, din*10, dout],
             [din, din*10, din*7, dout],
             [din, din*15, din*10, din*7, dout],
             [din, din*15, din*1.5, din*10, din*7, dout]]
arch_list_e=[[din, din*15, din*10, din*7, dout]]

y_out=hw_utils.testmodels(X_tr, y_tr, X_te, y_te, arch_list_e, actfn='relu', last_act='softmax', reg_coeffs=[0.0],
				num_epoch=30, batch_size=1000, sgd_lr=5e-4, sgd_decays=[0.0], sgd_moms=[0.99],sgd_Nesterov=False, EStop=False,
                verbose=0)
hw_utils.end_time(time_e)
validate[2]=y_out[0]
validate[3]=y_out[1]
#np.savetxt("validate_result.csv",y_out)
#np.savetxt("validate_result.csv",y_out)
